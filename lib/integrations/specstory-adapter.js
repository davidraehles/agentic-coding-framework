...
class SpecstoryAdapter {
  ...
  async initialize() {
    try {
      ...
      console.warn('Specstory extension not available:', error.message);
      return false;
    }
  }
  ...
  async logConversation(entry) {
    ...
    try {
      ...
      console.error('Failed to log via Specstory:', error);
      return false;
    }
  }
    ...
    for (const port of ports) {
      try {
        ...
        if (data.extensionId === this.extensionId || data.name === 'specstory') {
          console.log(`Connected to Specstory via HTTP on port ${port}`);
  ...
  async connectViaIPC() {
    ...
    try {
      ...
      if (connected) {
        console.log('Connected to Specstory via IPC');
  ...
  async connectViaFileWatch() {
    ...
    try {
      ...
      console.log('Using Specstory file watch directory');
...
module.exports = SpecstoryAdapter;