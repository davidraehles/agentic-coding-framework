...
class ClaudeMCPAdapter extends AgentAdapter {
  constructor(config = {}) {
    super(config);
    this.capabilities = ['mcp', 'memory', 'browser', 'logging'];
  ...
  // Memory operations - These will be handled by Claude's MCP tools
  async memoryCreate(entities) {
    // In Claude context, this would be handled via mcp__memory__create_entities
    // For now, we'll document that this requires Claude to be running
    console.log('Memory operations require Claude Code to be running with MCP');
    return { 
      success: true, 
      message: 'Use mcp__memory__create_entities tool in Claude'
    };
  }
  ...
  async memoryCreateRelations(relations) {
    ...
    return { 
      success: true, 
      message: 'Use mcp__memory__create_relations tool in Claude'
    };
  }
  ...
  async memorySearch(query) {
    ...
    return { 
      success: true, 
      message: 'Use mcp__memory__search_nodes tool in Claude'
    };
  }
  ...
  async memoryRead() {
    ...
    return { 
      success: true, 
      message: 'Use mcp__memory__read_graph tool in Claude'
    };
  }
