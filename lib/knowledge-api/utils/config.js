...
export class ConfigManager {
  ...
  _findProjectRoot() {
    ...
    while (currentDir !== path.dirname(currentDir)) {
      ...
      // Also check for any shared-memory-*.json files (team-specific or generic)
      try {
        const files = require('fs').readdirSync(currentDir);
        const hasSharedMemory = files.some(file => 
          file === 'shared-memory.json' || file.startsWith('shared-memory-')
        );
        if (hasSharedMemory) {
          return currentDir;
        }
  ...
  _resolveDefaultStoragePath() {
    ...
    if (codingKbPath) {
      ...
      if (projectRoot) {
        ...
        return path.join(codingKbPath, `shared-memory-${projectName}.json`);
      }
