...
export class ConfigManager {
  ...
  async save(configPath = null) {
    ...
    try {
      ...
      console.warn(`Failed to save config to ${targetPath}:`, error.message);
    }
  }
  ...
  _resolveDefaultStoragePath() {
    ...
    if (projectRoot) {
      // Use project-local shared-memory.json if it exists
      const projectStorage = path.join(projectRoot, 'shared-memory.json');
      try {
        accessSync(projectStorage);
    ...
    // Default to current directory or home directory
    const fallbackPaths = [
      './shared-memory.json',
      path.join(os.homedir(), '.knowledge-api', 'shared-memory.json')
    ];
  ...
  _resolveCodingStoragePath() {
    ...
    return path.join(os.homedir(), '.knowledge-api', 'shared-memory-coding.json');
  }
