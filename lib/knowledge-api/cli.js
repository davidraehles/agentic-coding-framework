...
async function removeRelation(api, options) {
  ...
  if (options.interactive) {
    ...
    const relationChoices = relations.map(r => ({
      name: `${r.from} -[${r.relationType}]-> ${r.to}`,
      value: r
    }));
  ...
  if (!relationData.from || !relationData.to || !relationData.relationType) {
    console.error(chalk.red('From entity, to entity, and relation type are required'));
    process.exit(1);
  }
