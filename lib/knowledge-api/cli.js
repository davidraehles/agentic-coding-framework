      ...
      return new Promise((resolve, reject) => {
        ...
        process.stdin.on('end', async () => {
          try {
            ...
            if (lines.length < 3) {
              console.error(chalk.red('Piped input for relation requires at least 3 lines:'));
              console.error(chalk.gray('  1. From entity name'));
              console.error(chalk.gray('  2. To entity name'));
              console.error(chalk.gray('  3. Relation type'));
              console.error(chalk.gray('  4. Significance (optional, default: 5)'));
              process.exit(1);
            }
            ...
            if (!entityNames.includes(fromEntity.trim())) {
              console.error(chalk.red(`From entity "${fromEntity}" not found`));
              console.error(chalk.gray('Available entities:'), entityNames.slice(0, 5).join(', ') + (entityNames.length > 5 ? '...' : ''));
              process.exit(1);
            }
            
            if (!entityNames.includes(toEntity.trim())) {
              console.error(chalk.red(`To entity "${toEntity}" not found`));
              console.error(chalk.gray('Available entities:'), entityNames.slice(0, 5).join(', ') + (entityNames.length > 5 ? '...' : ''));
              process.exit(1);
            }
            ...
            if (!validRelationTypes.includes(relationType.trim())) {
              console.error(chalk.red(`Invalid relation type: ${relationType}`));
              console.error(chalk.gray('Valid types:'), validRelationTypes.join(', '));
              process.exit(1);
            }
