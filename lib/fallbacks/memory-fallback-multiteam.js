...
class MemoryFallbackServiceMultiTeam {
  ...
  async initialize() {
    try {
      ...
      console.log(`Memory service initialized with ${this.graph.order} nodes and ${this.graph.size} edges (team: ${this.team})`);
    } catch (error) {
  ...
  async loadFromSharedMemory() {
    ...
    // Load team-specific or default file
    ...
    console.log(`Loaded knowledge from ${filesToLoad.map(f => path.basename(f.path)).join(', ')}`);
  }
  ...
  async _loadFromFile(filePath, source) {
    try {
      ...
      console.log(`Could not load ${path.basename(filePath)}: ${error.message}`);
    }
  }
  ...
  async syncToSharedMemory(force = false) {
    ...
    const knownFiles = [this.codingFilePath, this.teamFilePath, this.sharedMemoryPath];
  ...
  async _saveToFile(filePath, data) {
    ...
    try {
      ...
      console.log(`Skipping sync to ${path.basename(filePath)} - another process is writing`);
      return;
    }
    ...
    try {
      ...
      console.log(`Synced ${data.entities.length} entities and ${data.relations.length} relations to ${path.basename(filePath)}`);
      
    } catch (error) {
      console.error(`Failed to sync to ${path.basename(filePath)}:`, error.message);
    } finally {
  ...
  async cleanup() {
    ...
    if (this.initialized) {
      ...
      await this.syncToSharedMemory(true);
    }
  }
