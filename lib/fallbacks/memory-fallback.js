...
class MemoryFallbackService {
  ...
  async createEntities(entities) {
    ...
    // Sync to shared-memory.json (only if auto-sync enabled)
    if (this.autoSyncEnabled) {
      await this.syncToSharedMemory();
    }
  ...
  async syncToSharedMemory(force = false) {
    ...
    try {
      ...
      const entities = this.graph.mapNodes((node, attributes) => {
        // Find existing entity in shared memory to preserve structure
        const existingEntity = sharedMemory.entities.find(e => 
      ...
      this.graph.forEachEdge((edge, attributes, source, target) => {
        ...
        if (!relationMap.has(key)) {
          // Find existing relation in shared memory to preserve structure
          const existingRelation = sharedMemory.relations.find(r => 
