...
class MemoryFallbackService {
  ...
  async initialize() {
    try {
      ...
      // Start sync interval (every 5 minutes - reduced from 30 seconds to prevent excessive syncing)
  ...
  async syncToSharedMemory(force = false) {
    ...
    const lockFile = this.sharedMemoryPath + '.lock';
    try {
      await fs.writeFile(lockFile, process.pid.toString(), { flag: 'wx' });
    } catch (error) {
      // Another process is writing, skip this sync
      console.log('Skipping sync - another process is writing to shared-memory.json');
      return;
    }
