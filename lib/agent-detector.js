...
class AgentDetector {
  constructor() {
    this.detectionMethods = {
      antigravity: this.detectAntigravity.bind(this),
      claude: this.detectClaude.bind(this),
      copilot: this.detectCoPilot.bind(this)
    };
    this.capabilities = {
      antigravity: ['task-management', 'plan-management', 'dashboard', 'logging'],
  ...
  /**
   * Detect Antigravity Environment
   */
  async detectAntigravity() {
    try {
      // Check for ANTIGRAVITY.md or specific env var
      const hasMarker = await fileExists('ANTIGRAVITY.md');
      const isAntigravityEnv = process.env.ANTIGRAVITY_AGENT === 'true';
      return hasMarker || isAntigravityEnv;
    } catch (error) {
      return false;
    }
  }
  ...
  /**
   * Get the best available agent (prefer Antigravity)
   */
  async getBest() {
    const detected = await this.detectAll();

    // Prefer Antigravity if available
