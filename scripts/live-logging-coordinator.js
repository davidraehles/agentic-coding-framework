...
`;
  ...
  async enhanceStatusLine() {
    const statusLinePath = join(process.cwd(), 'scripts/combined-status-line.js');
    
    if (!existsSync(statusLinePath)) {
      console.warn('Status line script not found - skipping enhancement');
      return;
    }
    
    try {
      // Read current status line
      let statusContent = readFileSync(statusLinePath, 'utf8');
      
      // Check if already enhanced
      if (statusContent.includes('LiveLoggingCoordinator')) {
        console.log('Status line already enhanced with live logging');
        return;
      }
      
      // Add live logging integration
      const enhancement = `
// Live Logging Integration
import { captureToolInteraction } from '../.mcp-sync/tool-interaction-hook.js';

class EnhancedStatusLine extends CombinedStatusLine {
  async generateStatus() {
    const status = await super.generateStatus();
    
    // Capture this status generation as a tool interaction
    captureToolInteraction(
      { name: 'StatusLine', params: { type: 'status_generation' } },
      status,
      { timestamp: Date.now(), source: 'statusLine' }
    );
...
export { EnhancedStatusLine };
`;
      
      // Add enhancement to the file
      statusContent += enhancement;
      
      // Create backup first
      writeFileSync(statusLinePath + '.backup', readFileSync(statusLinePath));
      
      // Write enhanced version
      writeFileSync(statusLinePath, statusContent);
      
      console.log('✅ Status line enhanced with live logging integration');
      
    } catch (error) {
      console.error('Status line enhancement error:', error);
    }
  }
...
async function getGlobalCoordinator() {
...
async function finalizeGlobalSession() {
  if (globalCoordinator) {
    const summary = await globalCoordinator.finalizeSession();
...
export { getGlobalCoordinator, finalizeGlobalSession };