/**
 * OpenAI LLM Provider
 * Implementation of LLM interface using OpenAI's API
 */
...
export class OpenAIProvider extends LLMProviderInterface {
  constructor(config = {}) {
    ...
    this.logger = new Logger('openai-provider');
    this.client = null;
  ...
  initialize() {
    ...
    if (!apiKey) {
      throw new Error('OPENAI_API_KEY environment variable or config.apiKey is required');
    }
  ...
  async analyze(prompt, content, options = {}) {
    try {
      ...
      const result = this.parseResponse(response.choices[0].message.content, options);
